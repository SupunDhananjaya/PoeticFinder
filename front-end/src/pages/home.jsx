  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_API}/api/elastic/search`,
        {
          params: {
            metaphor: metaphor,
            lyrics: lyrics,
  ...
  useEffect(() => {
    const getRandPoem = async () => {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_API}/api/elastic/randompoem`
      );
      const data = res.data;
      setRandomPoem({
    ...
    <Grid container>
      ...
      >
        <Grid item xs={12}>
          <Paper
            variant="elevation"
            sx={{ textAlign: 'center', padding: '15px', fontSize: '25px' }}
          >
            <span className="blue-color">PoeticFinder</span> is{' '}
            <span className="green-color">easy</span> to use. Simply enter the{' '}
            <span className="green-color">keyword</span> or{' '}
            <span className="green-color">phrase</span> you are looking for in
            the search bar, and <span className="blue-color">PoeticFinder</span>{' '}
            will return a list of poems that match your query!
          </Paper>
        </Grid>
